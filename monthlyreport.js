document.addEventListener('DOMContentLoaded', () => {
    ...
    async function handleDownloadPdf() {
        ...
        // Revert back to canvas after PDF is generated
        if (radarChartInstance) {
    ...
    function renderPreview(data) {
        ...
        html += `
            <div class="report-section">
                ...
                <ul class="top-symptoms">${overview.top_symptoms.map(s => `<li>${s}</li>`).join('')}</ul>
        ...
        html += `
            <div class="report-section">
                ...
                <p><strong>Status:</strong> ${handleMarkup(insight.score_or_message)}</p>
                <p><strong>Tied to Logs:</strong></p>
                <ul>${insight.tied_to_logs.map(log => `<li>${handleMarkup(log)}</li>`).join('')}</ul>
                <p><strong>Likely Root Causes:</strong></p>
                <ul>${insight.likely_root_causes.map(cause => `<li>${handleMarkup(cause)}</li>`).join('')}</ul>
            </div>`;
        ...
        html += `
            <div class="report-section">
                ...
                <div class="logged-patterns-grid">
                    ${Object.entries(data.logged_patterns).map(([key, value]) => `
                        <div class="pattern-card">
                            <h5>${key.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}</h5>
                            <strong>Did Well:</strong>
                            <ul class="did-well">${value.did_well.map(i => `<li>${handleMarkup(i)}</li>`).join('')}</ul>
                            <strong>Areas to Improve:</strong>
                            <ul class="areas-to-improve">${value.areas_to_improve.map(i => `<li>${handleMarkup(i)}</li>`).join('')}</ul>
                        </div>
                    `).join('')}
                </div>
            </div>`;
        ...
        html += `
            <div class="report-section">
                ...
                <div class="action-steps-grid">
                    ${Object.entries(actions).filter(([key]) => key !== 'encouragement_message').map(([key, value]) => `
                        <div>
    ...
    function renderRadarChart(chartData) {
        ...
        radarChartInstance = new Chart(ctx, {
            type: 'radar',
            data: {
                labels: Object.keys(chartData.scores).map(key => key.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())),
                datasets: [{
    ...
    function buildFormSection(data, path, parentElement) {
        for (const key in data) {
            ...
            if (['string', 'number'].includes(typeof value)) {
                parentElement.appendChild(createField(currentPath, value, key, typeof value === 'string' ? 'textarea' : 'text'));
            } else if (Array.isArray(value)) {
                const fieldset = createFieldset(key);
                value.forEach((item, index) => {
                    const itemPath = `${currentPath}[${index}]`;
                    if (typeof item === 'string') {
                        fieldset.appendChild(createField(itemPath, item, `${key.replace(/s$/, '')} #${index + 1}`, 'textarea'));
                    } else if (typeof item === 'object' && item !== null) {
                        const itemFieldset = createFieldset(item.name || item.tag || `${key.replace(/s$/, '')} #${index + 1}`);
